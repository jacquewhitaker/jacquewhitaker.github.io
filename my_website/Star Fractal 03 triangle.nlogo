turtles-own [
  moves         ;; how many sides the turtle drew so far
  star-side     ;; length of the drawing side
  generation    ;; number of prior generations.
]               ;; Example: if the turtle was hatched (generation = 2) from
                ;; from a turtle that was hatched (generation = 1)
                ;; from the first turtle (generation = 0)

to setup        ;; (listener) runs when the setup button is clicked
  clear-all                       ;; clears the prior setup
  create-turtles 1 [              ;; constructs the turtle
    set size 10                   ;; so turtles are easy to see
    set ycor max-pycor * 0.9      ;; place near top of world
    set star-side ycor * 1.97538  ;; length of the sides of the star it will draw
    pen-down                      ;; get pen ready to draw
    set heading 180 + 30          ;; starting direction from North (+y axis)
    recolor                       ;; run the method to set the color
  ]
  reset-ticks                     ;; ??
end

to go            ;; (listener) runs when the go button is clicked
  if not any? turtles [ stop ]
  ask turtles [                          ;; gets which turtle object it is drawing
    fd star-side                         ;; draws the side
    rt 180 + 60                          ;; rotates to next direction
    if generation < fractal-level [      ;; If it's not drawing at the deepest level of the frsctal,
      hatch 1 [                          ;;    hatch a new turtle to draw at a lower level
        set generation generation + 1    ;; increment the generation
        set star-side star-side / 2      ;; change the side length
        recolor                          ;; change the color
        set moves 0                      ;; reset moves count
        rt 18                            ;; rotate next generation to point in new direction
      ]
    ]
    set moves moves + 1                  ;; increment the drawn side count (it draws 1 side per move command)
                                         ;; the move counter starts = 0
    if moves > 3 [ die ]                 ;; When the turtle has completed its star, it's done
  ]
  tick
end

to recolor  ;; turtle procedure
;;  set color scale-color pink (fractal-level - generation) -1 (fractal-level + 1)
  ifelse generation < 1
    [ set color pink ]                      ;; set first generation color to pink
    [ set color (color - (10 * (fractal-level))) ]  ;; change the color with each generation
end

; edited 2017 J Whitaker for educational purposes only
